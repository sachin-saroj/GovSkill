...
async def test_admin_cms_module_and_quiz_crud():
    ...
    try:
        ...
        async with async_session_test() as session:
            admin_user = User(email="admin_cms@gov.in", password_hash=get_password_hash("adminpass123"), role="admin")
            emp_user = User(email="emp_cms@gov.in", password_hash=get_password_hash("emppass123"), role="employee")
            session.add_all([admin_user, emp_user])
            await session.commit()

        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            # Login tokens
            admin_login = await client.post("/api/auth/login", json={"email": "admin_cms@gov.in", "password": "adminpass123"})
            admin_headers = {"Authorization": f"Bearer {admin_login.json()['access_token']}"}

            emp_login = await client.post("/api/auth/login", json={"email": "emp_cms@gov.in", "password": "emppass123"})
            ...
            create_q_resp = await client.post(
                ...
                json={
                    ...
                    "options": ["To filter unauthorized traffic", "To speed up internet connection", "To format documents", "To print pages"],
            ...
            admin_q_list = await client.get(f"/api/admin/modules/{module_id}/questions", headers=admin_headers)
            ...
            del_q_resp = await client.delete(f"/api/admin/questions/{question_id}", headers=admin_headers)
            assert del_q_resp.status_code == 204

            # 9. Admin Delete Module -> 204 NO CONTENT
            del_mod_resp = await client.delete(f"/api/admin/modules/{module_id}", headers=admin_headers)
