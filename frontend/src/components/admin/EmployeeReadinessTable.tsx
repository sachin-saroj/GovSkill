...
export const EmployeeReadinessTable: React.FC<EmployeeReadinessTableProps> = ({
  ...
  return (
    <Card className="p-0 overflow-hidden border-slate-200 shadow-civic-sm bg-white">
      ...
      <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-600 font-semibold">
          ...
          <button
            ...
            className="p-1.5 border border-slate-300 rounded-lg hover:bg-white disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-civic-xs"
          >
              ...
              {attempts.map((att, idx) => {
                ...
                return (
                  <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4 font-extrabold text-civic-900 font-mono">
                      {att.score} / {att.total}
                    </td>
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold border shadow-civic-xs ${
                          isPass
