      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-100 pb-5">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Award className="h-5 w-5 text-civic-700" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredList.map((item) => (
          <motion.div
            ...
            className="rounded-xl border border-slate-200/90 bg-slate-50/50 p-4.5 space-y-3.5 hover:border-slate-300 transition-colors"
          >
            {/* Top Row: Competency & Badges */}
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-0.5">
                <div className="text-[11px] font-bold uppercase tracking-wider text-civic-700">
                  {item.module_title}
                </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs font-semibold">
                <span className="text-slate-600">
                  Mastery Evidence:{' '}
                  <strong className="text-slate-900">{item.mastery_score}%</strong>
                </span>
                <span className="text-slate-400 text-[11px]">
                  {item.attempts_evaluated > 0
                    ? `${item.attempts_evaluated} assessment attempt${
            ...
            <div className="flex items-center gap-2 pt-1">
              <Link
                to={item.deep_link}
                className="inline-flex items-center gap-1 text-xs font-semibold text-civic-700 hover:text-civic-800 bg-civic-50 hover:bg-civic-100/80 px-2.5 py-1.5 rounded-lg border border-civic-200/80 transition-colors"
              >
                <BookOpen className="h-3 w-3" />
                {item.target_section_title ? `Review Section ${item.target_section_index + 1}` : 'Review Section'}
              </Link>
              <Link
                ...
                className="inline-flex items-center gap-1 text-xs font-semibold text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-100 px-2.5 py-1.5 rounded-lg border border-slate-200 shadow-2xs transition-colors"
              >
                <Bot className="h-3 w-3 text-civic-700" />
                Practice in Copilot
              </Link>
            </div>
          </motion.div>
